...
def update_output(n_clicks, log_content):
    ...
    try:
        ...
        # Create audit table with broker order IDs
...
def create_audit_table(audit_data):
    """Create HTML table for audit trail with Tailwind CSS including Broker Order ID."""
    # Create table header
    header = html.Thead(html.Tr([
        ...
        html.Th("Client Order ID", className="px-6 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"),
        html.Th("Broker Order ID", className="px-6 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"),
    ...
    for event in audit_data:
        ...
        row = html.Tr(className="hover:bg-gray-50", children=[
            ...
            html.Td(event['broker_order_id'] or '-', className="px-6 py-4 whitespace-nowrap text-sm text-blue-600 font-mono"),
