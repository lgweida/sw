...
def update_output(n_clicks, log_content):
    if n_clicks == 0 or not log_content:
        return ['0', 'N/A', 'N/A', '0', '0', '$0.00', 'Unknown', "No data to display", go.Figure(), '']
    ...
    try:
        ...
        # Create audit table with broker order IDs and avg_px
        ...
        return [f"Error: {str(e)}"] * 7 + [f"Error: {str(e)}", go.Figure(), '']
