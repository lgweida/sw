...
@dataclass
class OrderChain:
    """Represents a complete order chain with parent-child relationships"""
    original_order_id: str
    chain_orders: List[str]  # All order IDs in chronological order
    parent_child_map: Dict[str, str]  # child_id -> parent_id
    order_details: Dict[str, Dict[str, Any]]  # order_id -> order details
...
def build_order_genealogy(messages: List[FIXMessage]) -> Dict[str, OrderChain]:
    """Build complete order chains showing parent-child relationships"""
    order_chains = {}
    parent_child_map = {}  # child_order_id -> parent_order_id
    order_details = {}     # order_id -> order info
    
    # First pass: collect all orders and their basic info
    for msg in messages:
        if msg.msg_type_code in ['D', 'G', 'F']:  # New Order, Replace, Cancel
            cl_ord_id = msg.fields.get('11', '')
            orig_cl_ord_id = msg.fields.get('41', '')  # Original/Parent order ID
            
            if cl_ord_id:
                order_details[cl_ord_id] = {
                    'cl_ord_id': cl_ord_id,
                    'orig_cl_ord_id': orig_cl_ord_id,
                    'msg_type': msg.msg_type,
                    'msg_type_code': msg.msg_type_code,
                    'timestamp': msg.timestamp,
                    'order_qty': safe_float(msg.fields.get('38', '0')),
                    'order_type': msg.fields.get('40', ''),
                    'side': msg.fields.get('54', ''),
                    'symbol': msg.fields.get('55', msg.fields.get('48', '')),
                    'account': msg.fields.get('1', ''),
                    'price': safe_float(msg.fields.get('44', '0')),
                    'currency': msg.fields.get('15', ''),
                    'raw_fields': msg.fields
                }
                
                # Build parent-child relationship
                if orig_cl_ord_id and orig_cl_ord_id != cl_ord_id:
                    parent_child_map[cl_ord_id] = orig_cl_ord_id
    
    # Second pass: build order chains
    ...
    for order_id in order_details:
        ...
        # Find the root order (original order)
        root_order = find_root_order(order_id, parent_child_map)
        
        if root_order in processed_orders:
            continue
            
        # Build the complete chain from root
        chain_orders = build_chain_from_root(root_order, parent_child_map, order_details)
        
        # Mark all orders in this chain as processed
        processed_orders.update(chain_orders)
        
        # Create OrderChain object
        if chain_orders and root_order in order_details:
            root_details = order_details[root_order]
            final_order = chain_orders[-1]
            final_details = order_details[final_order]
            
            order_chain = OrderChain(
                original_order_id=root_order,
                chain_orders=chain_orders,
                parent_child_map={child: parent for child, parent in parent_child_map.items() 
                                if child in chain_orders or parent in chain_orders},
                order_details={oid: order_details[oid] for oid in chain_orders if oid in order_details},
                total_quantity=root_details.get('order_qty', 0),
                symbol=root_details.get('symbol', ''),
                side=root_details.get('side', ''),
                creation_time=root_details.get('timestamp', ''),
                final_time=final_details.get('timestamp', '')
            )
            
            order_chains[root_order] = order_chain
    
    return order_chains

def find_root_order(order_id: str, parent_child_map: Dict[str, str]) -> str:
    """Find the root (original) order in the chain"""
    current_order = order_id
    visited = set()
    
    while current_order in parent_child_map and current_order not in visited:
        visited.add(current_order)
        current_order = parent_child_map[current_order]
    
    return current_order

def build_chain_from_root(root_order: str, parent_child_map: Dict[str, str], order_details: Dict[str, Dict]) -> List[str]:
    """Build ordered chain starting from root order"""
    chain = [root_order]
    
    # Find all children of current orders in the chain
    while True:
        current_length = len(chain)
        current_orders = set(chain)
        
        # Look for children of any order currently in the chain
        for child, parent in parent_child_map.items():
            if parent in current_orders and child not in current_orders:
                # Insert child after its parent
                parent_index = chain.index(parent)
                chain.insert(parent_index + 1, child)
        
        # If no new orders were added, we're done
        if len(chain) == current_length:
            break
    
    # Sort by timestamp to ensure chronological order
    if all(order_id in order_details for order_id in chain):
        chain.sort(key=lambda oid: order_details[oid].get('timestamp', ''))
    
    return chain

def track_order_executions(messages: List[FIXMessage], order_chains: Dict[str, OrderChain]) -> Dict[str, List[Dict]]:
    """Track all executions for each order chain"""
    chain_executions = {}
    
    for chain_id, order_chain in order_chains.items():
        executions = []
        cumulative_qty = 0
        ...
        for msg in messages:
            if msg.msg_type_code == '8':  # Execution Report
                ...
                if cl_ord_id in order_chain.chain_orders:
                    ...
                    execution = {
                        ...
                        'order_status': msg.fields.get('39', ''),
                        'exec_type': msg.fields.get('150', ''),  # ExecType if available
                        'chain_position': order_chain.chain_orders.index(cl_ord_id) + 1,
                        'is_final_order': cl_ord_id == order_chain.chain_orders[-1]
                    }
                    
                    executions.append(execution)
                    cumulative_qty = max(cumulative_qty, cum_qty)
        
        chain_executions[chain_id] = {
            'executions': executions,
            'total_executed': cumulative_qty,
            'execution_count': len([e for e in executions if e['last_qty'] > 0])
        }
    
    return chain_executions
...
def print_order_genealogy(order_chains: Dict[str, OrderChain], chain_executions: Dict[str, List[Dict]]) -> None:
    """Print detailed order genealogy and execution tracking"""
    print(f"\n{'='*100}")
    print(f"COMPLETE ORDER GENEALOGY AND EXECUTION TRACKING")
    print(f"{'='*100}")
    
    print(f"\n📊 SUMMARY:")
    print(f"   • Total Order Chains: {len(order_chains)}")
    total_orders = sum(len(chain.chain_orders) for chain in order_chains.values())
    print(f"   • Total Orders (including replacements): {total_orders}")
    total_executions = sum(data['execution_count'] for data in chain_executions.values())
    print(f"   • Total Executions: {total_executions}")
    
    for i, (chain_id, order_chain) in enumerate(order_chains.items(), 1):
        print(f"\n{'-'*80}")
        print(f"ORDER CHAIN #{i}")
        print(f"{'-'*80}")
        
        print(f"🔗 Chain Overview:")
        print(f"   • Original Order ID: {order_chain.original_order_id}")
        print(f"   • Chain Length: {len(order_chain.chain_orders)} orders")
        print(f"   • Symbol: {order_chain.symbol}")
        print(f"   • Side: {'Buy' if order_chain.side == '1' else 'Sell' if order_chain.side == '2' else order_chain.side}")
        print(f"   • Total Quantity: {order_chain.total_quantity:,.0f}")
        print(f"   • Duration: {order_chain.creation_time} → {order_chain.final_time}")
        
        print(f"\n📋 Order Sequence:")
        for j, order_id in enumerate(order_chain.chain_orders, 1):
            order_info = order_chain.order_details.get(order_id, {})
            parent_id = order_chain.parent_child_map.get(order_id, '')
            
            prefix = "├── " if j < len(order_chain.chain_orders) else "└── "
            if j == 1:
                prefix = "┌── "
            ...
            order_type_desc = {
                ...
                'G': 'REPLACEMENT', 
                'F': 'CANCEL'
            }.get(order_info.get('msg_type_code', ''), 'UNKNOWN')
            
            print(f"   {prefix} #{j} {order_id}")
            print(f"   {'    ' if j < len(order_chain.chain_orders) else '    '} Type: {order_type_desc}")
            print(f"   {'    ' if j < len(order_chain.chain_orders) else '    '} Time: {order_info.get('timestamp', 'N/A')}")
            if parent_id:
                print(f"   {'    ' if j < len(order_chain.chain_orders) else '    '} Parent: {parent_id}")
            if order_info.get('price', 0) > 0:
                print(f"   {'    ' if j < len(order_chain.chain_orders) else '    '} Price: ${order_info.get('price', 0):.2f}")
            
        # Show executions for this chain
        if chain_id in chain_executions:
            exec_data = chain_executions[chain_id]
            executions = exec_data['executions']
            
            if executions:
                print(f"\n🎯 Executions ({len(executions)} total):")
                
                for exec in executions:
                    if exec['last_qty'] > 0:  # Only show actual fills
                        chain_pos = f"[Chain #{exec['chain_position']}]"
                        final_marker = " (FINAL ORDER)" if exec['is_final_order'] else ""
                        
                        print(f"   • {exec['timestamp']} {chain_pos}")
                        print(f"     Order: {exec['cl_ord_id']}{final_marker}")
                        print(f"     Fill: {exec['last_qty']:,.0f} @ ${exec['last_px']:.4f}")
                        print(f"     Cumulative: {exec['cum_qty']:,.0f} shares")
                        if exec['last_mkt']:
                            print(f"     Venue: {exec['last_mkt']}")
                        print()
                
                print(f"   📈 Chain Statistics:")
                print(f"     • Total Executed: {exec_data['total_executed']:,.0f} shares")
                print(f"     • Fill Rate: {(exec_data['total_executed'] / order_chain.total_quantity * 100):.1f}%")
                print(f"     • Remaining: {max(0, order_chain.total_quantity - exec_data['total_executed']):,.0f} shares")
            else:
                print(f"\n   ℹ️  No executions found for this order chain")
    
    print(f"\n{'='*100}")

def create_order_tracking_summary(order_chains: Dict[str, OrderChain], chain_executions: Dict[str, List[Dict]]) -> Dict[str, Any]:
    """Create a comprehensive order tracking summary"""
    summary = {
        'total_chains': len(order_chains),
        'chains': {},
        'statistics': {
            'total_orders': 0,
    ...
    for chain_id, order_chain in order_chains.items():
        # Count order types
        replacements = sum(1 for oid in order_chain.chain_orders 
                          if order_chain.order_details.get(oid, {}).get('msg_type_code') == 'G')
        cancellations = sum(1 for oid in order_chain.chain_orders 
                           if order_chain.order_details.get(oid, {}).get('msg_type_code') == 'F')
        
        # Get execution data
        exec_data = chain_executions.get(chain_id, {'total_executed': 0, 'executions': []})
        executed_qty = exec_data['total_executed']
        
        # Collect venues
        venues = set(exec['last_mkt'] for exec in exec_data['executions'] 
                    if exec.get('last_mkt'))
        
        chain_summary = {
            'original_order_id': order_chain.original_order_id,
            'chain_orders': order_chain.chain_orders,
            'order_count': len(order_chain.chain_orders),
            'replacements': replacements,
            'cancellations': cancellations,
            'symbol': order_chain.symbol,
            'side': order_chain.side,
            'total_quantity': order_chain.total_quantity,
            'executed_quantity': executed_qty,
            'remaining_quantity': max(0, order_chain.total_quantity - executed_qty),
            'fill_percentage': (executed_qty / order_chain.total_quantity * 100) if order_chain.total_quantity > 0 else 0,
            'creation_time': order_chain.creation_time,
            'final_time': order_chain.final_time,
            'venues_used': list(venues),
            'execution_count': len([e for e in exec_data['executions'] if e.get('last_qty', 0) > 0]),
            'parent_child_relationships': order_chain.parent_child_map
        }
        
        summary['chains'][chain_id] = chain_summary
        
        # Update statistics
        summary['statistics']['total_orders'] += len(order_chain.chain_orders)
        summary['statistics']['total_replacements'] += replacements
        summary['statistics']['total_cancellations'] += cancellations
        summary['statistics']['total_executed_quantity'] += executed_qty
        summary['statistics']['symbols_traded'].add(order_chain.symbol)
        summary['statistics']['venues_used'].update(venues)
        
        chain_lengths.append(len(order_chain.chain_orders))
    
    # Calculate averages
    if chain_lengths:
        summary['statistics']['average_chain_length'] = sum(chain_lengths) / len(chain_lengths)
    
    # Convert sets to lists for JSON serialization
    summary['statistics']['symbols_traded'] = list(summary['statistics']['symbols_traded'])
    summary['statistics']['venues_used'] = list(summary['statistics']['venues_used'])
...
def create_audit_trail(log_data: str) -> Dict[str, Any]:
    ...
    audit_trail = {
        'summary': {
            ...
            'remaining_quantity': max(0, original_qty - total_qty),
            'fill_percentage': (total_qty / original_qty * 100) if original_qty > 0 else 0,
            'vwap': round(vwap, 4),
            'total_executions': len([e for e in executions if e['last_qty'] > 0]),
...
def calculate_execution_metrics(audit_trail: Dict[str, Any]) -> Dict[str, Any]:
    ...
    prices = [exec['last_px'] for exec in executions if safe_float(str(exec['last_px'])) > 0]
    quantities = [exec['last_qty'] for exec in executions if safe_float(str(exec['last_qty'])) > 0]
    
    if not prices or not quantities:
        return {
            'total_executions': len(executions),
            'error': 'No valid price or quantity data found'
        }
    
    total_quantity = sum(quantities)
    total_value = sum(safe_float(str(exec['last_px'])) * safe_float(str(exec['last_qty'])) 
                     for exec in executions if safe_float(str(exec['last_px'])) > 0)
    ...
    metrics = {
        ...
        'quantity_range': {
            'min': min(quantities),
            'max': max(quantities),
            'average': total_quantity / len(quantities)
        },
        'venues': list(set(exec['last_mkt'] for exec in executions if exec['last_mkt'])),
        'venue_distribution': {}
    }
    ...
    for exec in executions:
        if exec['last_mkt'] and safe_float(str(exec['last_qty'])) > 0:
            venue_qty[exec['last_mkt']] += safe_float(str(exec['last_qty']))
