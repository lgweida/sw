    ...
    for line in lines:
        ...
        try:
            ...
            if msg_type in ['D', 'G', 'F', '8']:
                ...
                if event['cl_ord_id']:
                    orders[event['cl_ord_id']].append(event)
    ...
    for event in audit_data:
        if (event['msg_type'] == 'Execution Report' and 
            ...
            try:
                ...
                # by comparing with previous cumulative quantity
...
def update_output(n_clicks, log_content):
    if n_clicks == 0 or not log_content:
        return ['0', 'N/A', 'N/A', '0', '0', '$0.00', go.Figure(), '']
    
    try:
        orders = process_fix_log(log_content)
        audit_data = generate_audit_data(orders)
        
        # Calculate summary statistics
        total_messages = len(audit_data)
        order_chain = list(orders.keys())[0] if orders else 'N/A'
        ...
        fig = create_timeline_chart(audit_data)
...
def create_audit_table(audit_data):
    """Create HTML table for audit trail."""
    table_header = [
        html.Thead(html.Tr([
            html.Th("Time"), html.Th("Message Type"), html.Th("Order ID"),
            html.Th("Direction"), html.Th("Price"), html.Th("Quantity"),
            html.Th("Cum Qty"), html.Th("Venue"), html.Th("Status")
        ]))
    ]
