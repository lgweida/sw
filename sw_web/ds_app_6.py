...
# Top navigation bar with theme toggle (only element that changes theme)
                ...
                dbc.Nav(
                    [
                        ...
                        dbc.Button(
                            ...
                            color="link",
                            className="me-2 text-white" if theme == "dark" else "me-2"
                        ),
...
# Left sidebar with filters (always light theme)
def create_sidebar():
    return dbc.Col(
        [
            dbc.Card(
...
app.layout = dbc.Container(
    ...
    fluid=True,
    className="dbc p-0",
    style={"minHeight": "100vh", "backgroundColor": "#f8f9fa"}  # Light background for entire app
)
...
def update_theme(theme):
    ...
    return [
        create_top_nav(theme),  # Only the top nav changes theme
        dbc.Container(
            dbc.Row(
                [
                    create_sidebar(),  # Always light
                    create_main_content()  # Always light
                ],
                className="g-0"
            ),
            fluid=True,
...
def update_results(category, time_period, selected_domains):
    ...
    # Create visualizations with light theme colors
    line_color = "#636EFA"
