    ...
    try:
        if os.path.exists('domains.json'):
            with open('domains.json', 'r') as f:
                data = json.load(f)
                # Handle both old format (list of strings) and new format (dict with aliases)
...
app.layout = html.Div([
    ...
    # Website Dialog Modal (initially hidden) - Updated with domains list and aliases
    html.Div(
            ...
            html.Div(
                ...
                children=[
                    ...
                    html.Div([
                        ...
                        html.Div([
                            html.H4("Main Domains", className="text-lg font-semibold text-gray-700 mb-3"),
                            html.Div(
                        ...
                        html.Div([
                            html.H4("Add/Edit Main Domain", className="text-lg font-semibold text-gray-700 mb-3"),
                            html.Div([
                                html.Label("Website URL:", className="block text-sm font-medium text-gray-700 mb-2"),
                                dcc.Input(
                                    id="website-url-input",
                                    type="text",
                                    placeholder="Enter main domain URL (e.g., microsoft.com)",
                        ...
                        html.Div([
                            html.H4("Manage Aliases", className="text-lg font-semibold text-gray-700 mb-3"),
                            html.Div([
                                html.Label("Selected Domain:", className="block text-sm font-medium text-gray-700 mb-2"),
                                html.Div(
                                    id="selected-domain-display",
                                    className="text-sm text-gray-600 mb-4 p-2 bg-gray-50 rounded"
                                ),
                                html.Label("Add Alias:", className="block text-sm font-medium text-gray-700 mb-2"),
                                dcc.Input(
                                    id="alias-input",
                                    type="text",
                                    placeholder="Enter alias domain (e.g., linkedin.com)",
                                ...
                                html.Label("Current Aliases:", className="block text-sm font-medium text-gray-700 mb-2"),
                                html.Div(
                                    id="aliases-list-container",
                                    className="bg-gray-50 rounded-lg p-3 max-h-48 overflow-y-auto border border-gray-200",
                                    children=[
                                        html.Div("Select a main domain to manage aliases", className="text-gray-500 text-center p-4")
                                    ]
                                )
                            ], className="bg-white p-4 rounded-lg border border-gray-200")
    ...
    dcc.Store(id='selected-domain', data=None)
...
def update_domains_list(domains_data):
    if not domains_data:
        ...
        count_text = "Total: 0 domains"
        return domains_list, count_text
    
    # Create domain items
    domain_items = []
    for i, (main_domain, aliases) in enumerate(domains_data.items()):
        alias_count = len(aliases)
        alias_badge = html.Span(
        ...
        domain_item = html.Div([
            html.Div([
                html.Span(f"{i+1}.", className="font-semibold text-gray-600 mr-2"),
                html.Span(main_domain, className="text-gray-800 flex-1"),
                alias_badge
            ], className="flex items-center"),
            html.Div([
                ...
                # Aliases button
                html.Button(
                    "🔗",  # Link icon for aliases
                    id={'type': 'manage-aliases-button', 'index': i},
                    n_clicks=0,
    ...
    domains_list = html.Div(domain_items)
    count_text = f"Total: {len(domains_data)} domains"
    
    return domains_list, count_text

# Callback to handle domain selection for aliases management
@app.callback(
    [Output('selected-domain', 'data'),
     Output('selected-domain-display', 'children'),
     Output('aliases-list-container', 'children')],
    [Input({'type': 'manage-aliases-button', 'index': dash.ALL}, 'n_clicks'),
     Input('domains-store', 'data')],
    [State('domains-store', 'data')],
    prevent_initial_call=True
)
def handle_domain_selection(manage_clicks, domains_data, current_domains):
    ...
    if button_id.startswith('{"index":') and 'manage-aliases-button' in button_id:
        try:
            button_data = json.loads(button_id)
            domain_index = button_data['index']
            domain_keys = list(current_domains.keys())
            if 0 <= domain_index < len(domain_keys):
                selected_domain = domain_keys[domain_index]
                aliases = current_domains.get(selected_domain, [])
                
                # Display selected domain
                domain_display = html.Div([
                    html.Strong(selected_domain),
                    html.Span(f" ({len(aliases)} aliases)", className="text-gray-500 ml-2")
                ])
                
                # Create aliases list
                if aliases:
                    aliases_list = html.Div([
                        html.Div([
                            html.Span(alias, className="flex-1 text-sm"),
                            html.Button(
                                "🗑",
                                id={'type': 'delete-alias-button', 'domain': selected_domain, 'alias': alias},
                                n_clicks=0,
                                title="Delete alias",
                    ...
                    aliases_list = html.Div([
                        html.P("No aliases yet. Add aliases using the form above.", 
                               className="text-gray-500 text-center p-2 text-sm")
                    ])
                
                return selected_domain, domain_display, aliases_list
        except:
            pass
    
    return dash.no_update, dash.no_update, dash.no_update

# Callback to add aliases
@app.callback(
    [Output('domains-store', 'data', allow_duplicate=True),
     Output('alias-input', 'value'),
     Output('dialog-status-message', 'children', allow_duplicate=True)],
    Input('add-alias-button', 'n_clicks'),
    [State('alias-input', 'value'),
     State('selected-domain', 'data'),
     State('domains-store', 'data')],
    prevent_initial_call=True
)
def add_alias(add_clicks, alias_value, selected_domain, domains_data):
    if not add_clicks or not alias_value or not selected_domain:
        return dash.no_update, dash.no_update, dash.no_update
    
    cleaned_alias = clean_domain(alias_value)
    if not cleaned_alias:
        error_status = html.Div([
            html.Span("⚠", className="mr-2"),
            html.Span("Invalid alias URL format", className="text-red-600")
        ])
        return dash.no_update, dash.no_update, error_status
    
    # Check if alias already exists for any domain
    for domain, aliases in domains_data.items():
        if cleaned_alias in aliases:
            error_status = html.Div([
                html.Span("⚠", className="mr-2"),
                html.Span(f"Alias {cleaned_alias} already exists for {domain}", className="text-yellow-600")
            ])
            return dash.no_update, dash.no_update, error_status
    
    # Add alias to selected domain
    updated_domains = domains_data.copy()
    if selected_domain in updated_domains:
        if cleaned_alias not in updated_domains[selected_domain]:
            updated_domains[selected_domain].append(cleaned_alias)
            
            # Save to file
            if save_domains(updated_domains):
                success_status = html.Div([
                    html.Span("✅", className="mr-2"),
                    html.Span(f"Added alias {cleaned_alias} to {selected_domain}", className="text-green-600")
                ])
                return updated_domains, "", success_status
            else:
                error_status = html.Div([
                    html.Span("❌", className="mr-2"),
                    html.Span("Failed to save alias", className="text-red-600")
                ])
                return dash.no_update, dash.no_update, error_status
    
    return dash.no_update, dash.no_update, dash.no_update

# Callback to delete aliases
@app.callback(
    [Output('domains-store', 'data', allow_duplicate=True),
     Output('dialog-status-message', 'children', allow_duplicate=True)],
    Input({'type': 'delete-alias-button', 'domain': dash.ALL, 'alias': dash.ALL}, 'n_clicks'),
    [State('domains-store', 'data')],
    prevent_initial_call=True
)
def delete_alias(delete_clicks, domains_data):
    ctx = dash.callback_context
    if not ctx.triggered:
    ...
    if button_id.startswith('{"alias":') and 'delete-alias-button' in button_id:
        try:
            button_data = json.loads(button_id)
            domain_name = button_data['domain']
            alias_name = button_data['alias']
            
            if domain_name in domains_data and alias_name in domains_data[domain_name]:
                updated_domains = domains_data.copy()
                updated_domains[domain_name].remove(alias_name)
                
                # Save to file
                if save_domains(updated_domains):
                    success_status = html.Div([
                        html.Span("✅", className="mr-2"),
                        html.Span(f"Deleted alias {alias_name} from {domain_name}", className="text-green-600")
                    ])
                    return updated_domains, success_status
                else:
                    error_status = html.Div([
                        html.Span("❌", className="mr-2"),
                        html.Span("Failed to delete alias", className="text-red-600")
                    ])
...
@app.callback(
    [Output('dialog-state', 'data'),
     ...
     Output('selected-domain-display', 'children', allow_duplicate=True),
     Output('aliases-list-container', 'children', allow_duplicate=True)],
...
def handle_dialog_operations(add_clicks, close_clicks, cancel_clicks, edit_clicks, delete_clicks, submit_clicks, clear_clicks,
    ...
    if button_id == 'add-website-button':
        return True, "Manage Website Domains", "", -1, "Add Website", dash.no_update, "", None, "No domain selected", html.Div("Select a main domain to manage aliases", className="text-gray-500 text-center p-4")
    
    # Handle dialog closing
    elif button_id in ['close-dialog-button', 'cancel-dialog-button']:
        return False, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, "", None, "No domain selected", html.Div("Select a main domain to manage aliases", className="text-gray-500 text-center p-4")
    ...
    # Handle edit button
        ...
        try:
            ...
            domain_keys = list(current_domains.keys())
            if 0 <= domain_index < len(domain_keys):
                domain_value = domain_keys[domain_index]
                return dash.no_update, dash.no_update, domain_value, domain_index, "Update Website", dash.no_update, "", dash.no_update, dash.no_update, dash.no_update
        except:
            pass
        return dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update
    ...
    # Handle delete button
        ...
        try:
            ...
            domain_keys = list(current_domains.keys())
            if 0 <= domain_index < len(domain_keys):
                deleted_domain = domain_keys[domain_index]
                updated_domains = current_domains.copy()
                del updated_domains[deleted_domain]
                ...
                if save_domains(updated_domains):
                    ...
                    return dash.no_update, dash.no_update, "", -1, "Add Website", updated_domains, success_status, None, "No domain selected", html.Div("Select a main domain to manage aliases", className="text-gray-500 text-center p-4")
                else:
                    error_status = html.Div([
                        html.Span("❌", className="mr-2"),
    ...
    # Handle clear form button
        ...
        return dash.no_update, dash.no_update, "", -1, "Add Website", dash.no_update, "", dash.no_update, dash.no_update, dash.no_update
    ...
    # Handle form submission
        ...
        if edit_index == -1:  # Add mode
            ...
            domain_keys = list(current_domains.keys())
            if 0 <= edit_index < len(domain_keys):
                old_domain = domain_keys[edit_index]
                if cleaned_domain in current_domains and cleaned_domain != old_domain:
                    duplicate_status = html.Div([
                        html.Span("⚠", className="mr-2"),
                        html.Span("Domain already exists", className="text-yellow-600")
                    ])
        ...
        if edit_index == -1:  # Add mode
            updated_domains = current_domains.copy()
            updated_domains[cleaned_domain] = []
            ...
            if 0 <= edit_index < len(domain_keys):
                old_domain = domain_keys[edit_index]
                updated_domains = current_domains.copy()
                aliases = updated_domains.pop(old_domain)
                updated_domains[cleaned_domain] = aliases
...
def render_content(tab, data, domains_data):
    ...
    if tab == 'tab-4':
        # Calculate total aliases count
        total_aliases = sum(len(aliases) for aliases in domains_data.values())
        ...
        return html.Div([
            ...
            html.Div([
                html.Div([
                    html.Div([
                        html.H3("Main Domains", className="text-lg font-semibold text-gray-700"),
                ...
                html.Div([
                    html.Div([
                        html.H3("Total Aliases", className="text-lg font-semibold text-gray-700"),
                        html.P(f"{total_aliases}", className="text-3xl font-bold text-green-600")
                    ], className="p-4")
            ...
            # Domains list with aliases
            html.Div([
                html.H3("Current Domains & Aliases", className="text-xl font-semibold text-gray-800 mb-4"),
                html.Div([
                    html.Div([
                        html.Div([
                            html.Span(f"{i+1}.", className="font-semibold text-gray-600 mr-2"),
                            html.Span(main_domain, className="text-gray-800 font-medium"),
                            html.Span(
                        ...
                        html.Div([
                            html.Span("Aliases: ", className="text-sm text-gray-600 font-medium"),
                            html.Span(
                ...
                html.Div([
                    ...
                    html.Ul([
                        html.Li("Click 'Manage Websites' to add, edit, or delete main domains and aliases", className="text-gray-600"),
                        html.Li("Main domains can have multiple aliases (e.g., MSFT → linkedin.com, github.com)", className="text-gray-600"),
                        html.Li("Aliases are automatically cleaned (removes http/https, www)", className="text-gray-600"),
                        html.Li("Duplicate aliases across different main domains are not allowed", className="text-gray-600"),
        ...
        return html.Div([
            ...
            html.Div([
                ...
                html.Div([
                    html.Div([
                        html.H3("Total Aliases", className="text-lg font-semibold text-gray-700"),
                        html.P(f"{total_aliases}", className="text-3xl font-bold text-indigo-600")
                    ], className="p-4")
            ...
            html.Div([
                ...
                html.P(f"Currently managing {len(domains_data)} main domains with {total_aliases} aliases."),
