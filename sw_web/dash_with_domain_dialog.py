...
def clean_domain(url):
    ...
    try:
        parsed = urlparse(url)
        domain = parsed.netlify if parsed.netlify else parsed.hostname
    ...
    html.Div(
        ...
        children=[
            html.Div(
                ...
                children=[
                    html.Div([
                        html.H3("Add New Website", className="text-xl font-bold text-gray-800 mb-4"),
                        html.Button(
                            "×", 
...
def handle_dialog_open_close(add_clicks, close_clicks, cancel_clicks, dialog_open):
...
# Callback to handle website submission
@app.callback(
...
def handle_website_submission(submit_clicks, url_value, current_domains):
    ...
    if save_domains(updated_domains):
        success_status = html.Div([
            html.Span("✅", className="mr-2"),
            html.Span(f"Added {cleaned_domain} successfully!", className="text-green-600")
        ])
