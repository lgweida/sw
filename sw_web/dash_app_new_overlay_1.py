...
def update_data_via_api_background():
    ...
    try:
        ...
        # Simulate API call delay - increased to 8 seconds for better visualization
        for i in range(600):
            if not background_task_running:  # Check if task was cancelled
                return
            time.sleep(1)
            # Update progress message
            elapsed = (datetime.now() - background_task_start_time).total_seconds()
            background_task_message = f"Processing... {int(elapsed)}/600 seconds completed"
        ...
        if os.path.exists(last_update_file):
            ...
            if last_update_month == current_month:
                ...
                background_task_message = "Data already updated this month."
                return
        ...
        background_task_message = f"Data updated successfully for {current_month}! Added {len(new_month_data)} new records."
                ...
                children=[
                    ...
                    html.Div([
                        html.Div([
                            ...
                            html.Span("Connecting to API server", className="text-sm", id="step-1-text"),
                        ...
                        html.Div([
                            ...
                            html.Span("Fetching monthly data", className="text-sm", id="step-2-text"),
                        ...
                        html.Div([
                            ...
                            html.Span("Processing records", className="text-sm", id="step-3-text"),
                        ...
                        html.Div([
                            ...
                            html.Span("Updating Excel file", className="text-sm", id="step-4-text"),
                        ...
                        html.Div([
                            ...
                            html.Span("Finalizing update", className="text-sm", id="step-5-text"),
...
def update_progress_steps(n_intervals, loading):
    ...
    step_thresholds = [30, 120, 240, 480, 600]
...
def update_progress_message(n_intervals, loading):
    ...
    # Calculate estimated time remaining based on 600 seconds total
    total_estimated = 600
    remaining = max(0, total_estimated - elapsed_seconds)
    
    # Format time strings
    elapsed_str = f"{elapsed_seconds//60:02d}:{elapsed_seconds%60:02d}"
    remaining_str = f"{remaining//60:02d}:{remaining%60:02d}"
    
    # Create progress message with more details
    progress_percent = min(100, int((elapsed_seconds / total_estimated) * 100))
    
    return html.Div([
        html.Div(f"Progress: {progress_percent}%", className="font-semibold"),
        html.Div([
            html.Span(f"Elapsed: {elapsed_str}", className="mr-4"),
            html.Span(f"Remaining: ~{remaining_str}")
