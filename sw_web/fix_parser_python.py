...
def create_audit_trail(log_data: str) -> Dict[str, Any]:
    ...
    for msg in messages:
        if msg.msg_type_code == '8':  # Execution Report
            last_qty = float(msg.fields.get('32', '0'))
            last_px = float(msg.fields.get('31', '0'))
            cum_qty = float(msg.fields.get('14', '0'))
    ...
    # Calculate VWAP
    vwap = total_value / sum(exec['last_qty'] for exec in executions if exec['last_qty'] > 0) if executions else 0
    
    # Get order details from first order
    first_order = next((msg for msg in messages if msg.msg_type_code == 'D'), None)
    original_qty = float(first_order.fields.get('38', '0')) if first_order else 0
