...
def update_display(parsed_data, msgtype_filter, sender_filter, target_filter, 
                   global_search, clear_clicks, source_data):
...
# Callback for export functionality (FIX Log)
@app.callback(
...
def export_data(n_clicks, parsed_data):
...
# Callback for Account Mapping page
@app.callback(
...
def update_account_display(tab_value, search_term, clear_clicks, network_data_dict, last_updated_str):
        ...
        if current_df is None or current_df.empty:
            ...
            if filtered_df.empty:
                ...
                if not filtered_df.empty:
                    ...
                    download_data = dict(content=csv_string, filename=filename)
                
                # Create download button and table container
                table_content = html.Div([
                    html.Div([
    ...
    return table_content, search_value, last_updated, stats_summary, tab_children, search_terms_display, download_data

# Callback for Account Mapping download button
@app.callback(
    Output('download-account-csv', 'data'),
