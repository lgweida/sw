...
# Prepare results
matched_records = []
matched_records.append(header + ";MAPPED_ID;MATCHED_FIELD")  # Add mapping ID and matched field to header

# Track matches by field
matches_by_sender = Counter()
matches_by_onbehalf = Counter()

for line in data_lines:
    fields = line.split(';')
    sendercomp = fields[0]  # SENDERCOMPID is the first field
    onbehalfof = fields[1]  # ONBEHALFOFCOMPID is the second field
    
    # Check SENDERCOMPID first (if not '*')
    if sendercomp != '*' and sendercomp in mapping_dict:
        matched_records.append(line + ";" + mapping_dict[sendercomp] + ";SENDERCOMPID")
        matches_by_sender[sendercomp] += 1
    
    # Then check ONBEHALFOFCOMPID (if not '*')
    elif onbehalfof != '*' and onbehalfof in mapping_dict:
        matched_records.append(line + ";" + mapping_dict[onbehalfof] + ";ONBEHALFOFCOMPID")
        matches_by_onbehalf[onbehalfof] += 1
...
print(f"\nTotal matches: {len(matched_records)-1}")
print(f"Matches by SENDERCOMPID: {sum(matches_by_sender.values())}")
print(f"Matches by ONBEHALFOFCOMPID: {sum(matches_by_onbehalf.values())}")

print("\nDetailed matches by SENDERCOMPID:")
for comp_id, count in sorted(matches_by_sender.items()):
    print(f"  {comp_id}: {count} matches (mapped to {mapping_dict[comp_id]})")

print("\nDetailed matches by ONBEHALFOFCOMPID:")
for comp_id, count in sorted(matches_by_onbehalf.items()):
    print(f"  {comp_id}: {count} matches (mapped to {mapping_dict[comp_id]})")

# Show sample of matches by type
print("\n" + "="*120)
print("SAMPLE RECORDS BY MATCH TYPE:")
print("="*120)

print("\nRecords matched by SENDERCOMPID (first 3):")
sender_samples = [r for r in matched_records[1:] if ";SENDERCOMPID" in r][:3]
for sample in sender_samples:
    print(f"  {sample}")

print("\nRecords matched by ONBEHALFOFCOMPID (first 3):")
onbehalf_samples = [r for r in matched_records[1:] if ";ONBEHALFOFCOMPID" in r][:3]
for sample in onbehalf_samples:
    print(f"  {sample}")