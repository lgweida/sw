...
app.layout = html.Div([
    ...
    # Website Dialog Modal (initially hidden)
    html.Div(
        ...
        children=[
            html.Div(
                ...
                children=[
                    html.Div([
                        html.H3("Add New Website", id="dialog-title", className="text-xl font-bold text-gray-800 mb-4"),
                        html.Button(
                            "×", 
...
# Callback to open dialog for add or edit
@app.callback(
...
def handle_dialog_open_close(add_clicks, close_clicks, cancel_clicks, edit_clicks, dialog_open, edit_index, domains_data):
    ...
    if button_id == 'add-website-button':
        return True, "Add New Website", "", -1, "Add Website"
    elif button_id.startswith('{"index":'):
        # This is an edit button - parse the index from the button ID
        try:
            button_data = json.loads(button_id)
            domain_index = button_data['index']
            # Pre-fill the input with the current domain value
            domain_value = domains_data[domain_index] if domain_index < len(domains_data) else ""
            return True, "Edit Website", domain_value, domain_index, "Update Website"
        except:
            return dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update
    elif button_id in ['close-dialog-button', 'cancel-dialog-button']:
        return False, dash.no_update, dash.no_update, dash.no_update, dash.no_update
    
    return dialog_open, dash.no_update, dash.no_update, dash.no_update, dash.no_update
...
# Callback to handle website submission (both add and edit)
@app.callback(
    [Output('domains-store', 'data'),
     Output('dialog-state', 'data', allow_duplicate=True),
     Output('website-url-input', 'value'),
...
def handle_website_submission(submit_clicks, url_value, current_domains, edit_index):
    if not submit_clicks or not url_value:
    ...
    # Check for duplicates (skip if we're editing the same domain)
    if edit_index == -1:  # Add mode
        ...
        # Check if the new domain already exists (and it's not the one we're editing)
    ...
    if save_domains(updated_domains):
        ...
        return updated_domains, False, "", success_status
    else:
        error_status = html.Div([
            html.Span("❌", className="mr-2"),
        ...
        return html.Div([
            ...
            html.Div([
                ...
                html.Div([
                    ...
                    html.Ul([
                        html.Li("Click 'Add New Website' to add domains to domains.json", className="text-gray-600"),
                        html.Li("Click 'Edit' to modify existing domains", className="text-gray-600"),
                        html.Li("Domains are automatically cleaned (removes http/https, www)", className="text-gray-600"),
                        html.Li("Duplicate domains are not added", className="text-gray-600"),
