...
def trigger_data_fetch(n_clicks, store_data):
    if n_clicks > store_data.get('clicks', 0):
        try:
            ...
            toast = html.Div([
                html.Div([
                    html.Div([
                        ...
                        html.Div([
                            html.Div("Success!", className="font-bold text-gray-900"),
                            html.Div("Data updated successfully!", className="text-gray-700 text-sm")
                        ])
            ...
            ], className="bg-white rounded-lg shadow-lg p-4 border-l-4 border-green-500 animate-fadeIn")
            
            return success_dot_class, "Updated", normal_overlay_class, success_icon_class, {'clicks': n_clicks}, toast
            ...
            toast = html.Div([
                html.Div([
                    html.Div([
                        html.I(className="fas fa-exclamation-triangle text-red-500 text-xl mr-3"),
                        html.Div([
                            html.Div("Error!", className="font-bold text-gray-900"),
                            html.Div(f"Update failed: {str(e)}", className="text-gray-700 text-sm")
                        ])
                    ], className="flex items-center"),
                    html.Button(
                        html.I(className="fas fa-times"),
            ...
            ], className="bg-white rounded-lg shadow-lg p-4 border-l-4 border-red-500 animate-fadeIn")
            
            return error_dot_class, "Error", normal_overlay_class, error_icon_class, {'clicks': n_clicks}, toast
    
    return dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update

# Callback to show/hide toast
@app.callback(
    Output("toast-container", "className"),
    [Input("update-button", "n_clicks"),
     Input("close-toast", "n_clicks")],
    [State("toast-container", "className")],
    prevent_initial_call=True
)
def toggle_toast(update_clicks, close_clicks, current_class):
