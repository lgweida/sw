...
def make_links_clickable(text):
    ...
    text_with_links = re.sub(url_regex, r'<a href="\1" target="_blank" class="text-blue-600 hover:text-blue-800 underline">\1</a>', text)
...
# Callback to update the refresh trigger
@app.callback(
...
def update_refresh_trigger(save_clicks, delete_clicks, current_data):
    # Increment the refresh trigger to signal that data has changed
...
def update_sidebar(search_term, clear_clicks, refresh_clicks, refresh_trigger):
...
def update_activities_list(refresh_clicks, search_term, refresh_trigger):
    df_activities = get_activities()
    ...
    for _, activity in df_activities.iterrows():
        ...
        if activity['links']:
            ...
            links_html = html.Div([
                ...
                html.Ul([
                    html.Li([
                        html.A(
                            link if len(link) < 50 else link[:47] + "...", 
                            href=link, 
        ...
        # Make URLs in details clickable
        details_with_links = make_links_clickable(activity['details'])
            ...
            html.Div([
                html.Div([
                    html.Div([
                        ...
                        html.Div([
                            dcc.Markdown(details_with_links, className="mt-2 text-gray-600", link_target="_blank")
...
# Callback to save activity and trigger refresh
@app.callback(
...
def save_activity_callback(n_clicks, current_activity, date, title, category, details, links, refresh_data):
    if n_clicks:
        if not all([date, title, category, details]):
            return "Please fill in all required fields", "inline-block ml-4 text-sm text-red-600", dash.no_update
        ...
        return message, "inline-block ml-4 text-sm text-green-600", refresh_data + 1
    
    return "", "inline-block ml-4 text-sm text-green-600", dash.no_update
...
# Callback to delete an activity and trigger refresh
@app.callback(
...
def delete_activity_callback(n_clicks, button_ids, refresh_data):
    if not any(n_clicks):
    ...
    if button_id:
        ...
        # Trigger a refresh by incrementing the refresh trigger
        return update_activities_list(None, None, refresh_data), refresh_data + 1
